...
IN_CSV = "drivers_2025.csv"          # must have at least: 'url' (title optional)
OUT_CSV = "drivers_2025_aggregated.csv"
HEADERS = {"User-Agent": "Mozilla/5.0"}

# ---------------- Driver/team list (2025, as provided) ----------------
DRIVER_TEAMS_2025 = {
...
DRIVER_NAMES_2025 = list(DRIVER_TEAMS_2025.keys())

# Build initials map from 2025 names (e.g., "LN" -> ["Lando Norris"])
INITIALS_TO_NAMES = defaultdict(list)
for name in DRIVER_NAMES_2025:
    parts = name.split()
    if len(parts) >= 2:
...
def scrape_article(url: str):
    ...
    for p in body.find_all("p"):
        ...
        if not name_full:
            continue  # not a recognized 2025 driver

        team_final = team or DRIVER_TEAMS_2025.get(name_full, "")
