...
IN_CSV = "drivers_2023.csv"          # must have at least: 'url' (title optional)
OUT_CSV = "drivers_2023_aggregated.csv"
HEADERS = {"User-Agent": "Mozilla/5.0"}

# ---------------- Driver/team list (2023, as provided) ----------------
DRIVER_TEAMS_2023 = {
...
DRIVER_NAMES_2023 = list(DRIVER_TEAMS_2023.keys())

# Build initials map from 2023 names (e.g., "LN" -> ["Lando Norris"])
INITIALS_TO_NAMES = defaultdict(list)
for name in DRIVER_NAMES_2023:
    parts = name.split()
    if len(parts) >= 2:
...
def scrape_article(url: str):
    ...
    for p in body.find_all("p"):
        ...
        if not name_full:
            continue  # not a recognized 2023 driver

        team_final = team or DRIVER_TEAMS_2023.get(name_full, "")
