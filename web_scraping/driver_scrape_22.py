...
IN_CSV = "drivers_2022.csv"          # must have at least: 'url' (title optional)
OUT_CSV = "drivers_2022_aggregated.csv"
HEADERS = {"User-Agent": "Mozilla/5.0"}

# ---------------- Driver/team list (2022, as provided) ----------------
DRIVER_TEAMS_2022 = {
...
DRIVER_NAMES_2022 = list(DRIVER_TEAMS_2022.keys())

# Build initials map from 2022 names (e.g., "LN" -> ["Lando Norris"])
INITIALS_TO_NAMES = defaultdict(list)
for name in DRIVER_NAMES_2022:
    parts = name.split()
    if len(parts) >= 2:
...
def scrape_article(url: str):
    ...
    for p in body.find_all("p"):
        ...
        if not name_full:
            continue  # not a recognized 2022 driver

        team_final = team or DRIVER_TEAMS_2022.get(name_full, "")
